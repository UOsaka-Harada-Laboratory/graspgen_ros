...
class GraspPlanningServer(Node):
    def __init__(self):
        ...
        self.planning_service = self.create_service(Empty, "plan_grasp", self.plan_grasp_callback)
        self.get_logger().info("Grasp planning service '/plan_grasp' ready.")
    ...
    def plan_grasp_callback(self, request, response):
        self.get_logger().info(f"Running grasp planner on: {self.mesh_path}")
    ...
    def publish_tf_and_marker(self, grasps, scores):
        ...
        obj_marker.scale.x = self.mesh_scale
        obj_marker.scale.y = self.mesh_scale
        obj_marker.scale.z = self.mesh_scale
        obj_marker.color.r = 0.7
        obj_marker.color.g = 0.7
        obj_marker.color.b = 0.7
        obj_marker.color.a = 1.0
        obj_marker.mesh_resource = 'file://' + self.mesh_path
